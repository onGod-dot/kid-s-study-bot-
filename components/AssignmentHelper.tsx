...
import { Upload, Camera, FileText, CheckCircle, XCircle, Loader2, ChevronDown, ChevronUp, RotateCcw, Download } from 'lucide-react'
...
export default function AssignmentHelper({ theme, ageGroup, avatarId }: AssignmentHelperProps) {
  ...
  // ── Download answer as PDF using browser print ────────────────────────────
  const downloadPDF = () => {
    if (!fullResponse) return
    const title = ageGroup === 'kids' ? 'Homework Answer' : 'Assignment Solution'
    const date = new Date().toLocaleDateString()

    // Strip markdown symbols for clean plain-text PDF
...
<head>
  ...
  <style>
    @page { margin: 2cm; }
    body { font-family: Georgia, serif; font-size: 13pt; line-height: 1.7; color: #111; }
    h1 { font-size: 18pt; margin-bottom: 4px; }
    .meta { color: #666; font-size: 10pt; margin-bottom: 24px; border-bottom: 1px solid #ddd; padding-bottom: 8px; }
    .step { margin-bottom: 18px; }
    .step-num { font-weight: bold; color: #333; }
    pre { white-space: pre-wrap; font-family: inherit; margin: 0; }
  </style>
</head>
<body>
  <h1>🎓 ${title}</h1>
  <div class="meta">Generated by Buddy AI · ${date}</div>
  <pre>${clean}</pre>
</body>
</html>`
  ...
  const downloadImage = async (imageUrl: string) => {
    try {
      ...
      a.download = 'buddy-assignment-image.png'
      a.click()
  ...
  return (
    <div className="space-y-5">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        ...
        {uploadedImage && (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="mt-4 space-y-2">
            <img
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 text-white/70 hover:text-white text-xs font-medium transition-all w-full justify-center border border-white/10"
            >
